...
; Keys
(deftest test-del
  (let [redis (mock/->redis {:foo 1 :bar 2 :quux 3 :qwiz 4})]
    (is (= 1 (r/del redis [:foo])))
    (is (= {:bar 2 :quux 3 :qwiz 4} @(.store redis)))
    (is (= 2 (r/del redis [:bar :quux :fake-1 :fake-2])))
    (is (= {:qwiz 4} @(.store redis)))))
...
(deftest test-hexists
  (let [redis (mock/->redis {:foo {:bar 1}})]
    (is (= true (r/hexists redis :foo :bar)))
    (is (= false (r/hexists redis :foo :fake-field)))
    (is (= false (r/hexists redis :fake-key :fake-field)))))
